...
public class GunScript : MonoBehaviour {
	...
	*/
	IEnumerator AnimationMeeleAttack(){
        ...
        Debug.DrawRay(barrel.transform.position, fwd * .8f, Color.red,3);
        if (Physics.Raycast(transform.position, fwd, out hit, .8f))
        {
            switch(hit.collider.gameObject.layer){
                case 8:
                    hit.collider.gameObject.GetComponent<EnemyControl>().HP -= damage/2;
                    Instantiate(bloodEffect, hit.point + (hit.normal * .00001f), Quaternion.LookRotation(hit.normal));
                    break;
                case 11:
                    break;
            }
                ...
                if (GD.clip > 0)
                {
                    ...
                    if (bullet)
                    {
                        ...
                        if (Physics.Raycast(transform.position, fwd, out hit, 10))
                        {
                            ...
                            {
                                case 8:
                                    hit.collider.gameObject.GetComponent<EnemyControl>().HP -= damage;
                                    Instantiate(bloodEffect, hit.point + (hit.normal * .00001f), Quaternion.LookRotation(hit.normal));
                                    break;
                                case 10:
                                    hit.collider.gameObject.GetComponent<ParticleSystem>().Play();
                                    hit.collider.gameObject.GetComponentInParent<EnemyControl>().HP -= damage*3;
                                    Instantiate(bloodEffect, hit.point + (hit.normal * .00001f), Quaternion.LookRotation(hit.normal));
                                    break;
                                case 11:
                                    GameObject bullHole = (GameObject)Instantiate(bulletHole, hit.point + (hit.normal * .00001f), Quaternion.LookRotation(hit.normal));
                                    bullHole.transform.parent = hit.collider.gameObject.transform;
                                    break;
                            }
                        }
                        else if (Physics.Raycast(transform.position, fwd, out hit, 50))
                        {
                            switch (hit.collider.gameObject.layer)
                            {
                                case 8:
            ...
            else if (GD.isFULL)
            {
                if (GD.clipFULL > 0)
                {
                    ...
                    if (bullet)
                    {
                        ...
                        else if (Physics.Raycast(transform.position, fwd, out hit, 25))
                        {
                            switch (hit.collider.gameObject.layer)
                            {
                                case 8:
	...
	*/
	...
	IEnumerator Reload_Animation(){
		if(GD.ammo > 0 && GD.clip < amountOfBulletsPerLoad && !reloading/* && !aiming*/){
			...
			if (meeleAttack == false && pmS.maxSpeed != runningSpeed) {
                ...
                /*if (GD.ammo - amountOfBulletsPerLoad >= 0) {
					GD.ammo -= (int)amountOfBulletsPerLoad - GD.ammo;
					GD.clip = (int)amountOfBulletsPerLoad;
				} else if (GD.ammo - (int)amountOfBulletsPerLoad < 0) {
					float valueForBoth = amountOfBulletsPerLoad - GD.clip;
					if (GD.ammo - valueForBoth < 0) {
						GD.clip += GD.ammo;
						GD.ammo = 0;
					} else {
						GD.ammo -= (int)valueForBoth;
						GD.clip += (int)valueForBoth;
					}
				}*/
                if (!GD.isFULL)
                {
                    if (GD.ammo >= 1)
                    {
                        int takeFromAmmo = 30 - GD.clip;
                        if (GD.ammo <= -1)
                        {
                            GD.ammo = 0;
                        }
                        else if (GD.ammo >= 0 && GD.ammo <= 8)
                        {
                            GD.clip += GD.ammo;
                            GD.ammo = 0;
                        }
                        ...
                        {
                            GD.ammo -= takeFromAmmo;
                            GD.clip += takeFromAmmo;
                ...
                else if (GD.isFULL)
                {
                    if (GD.ammoFULL >= 1)
                    {
                        int takeFromAmmo = 30 - GD.clipFULL;
                        if (GD.ammoFULL <= -1)
                        {
                            GD.ammoFULL = 0;
                        }
                        else if (GD.ammoFULL >= 0 && GD.ammoFULL <= 8)
                        {
                            GD.clipFULL += GD.ammoFULL;
                            GD.ammoFULL = 0;
                        }
                        else
                        {
                            GD.ammoFULL -= takeFromAmmo;
                            GD.clipFULL += takeFromAmmo;
